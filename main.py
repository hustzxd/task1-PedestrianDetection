...
def main():
    ...
    if 'L0' in args.arch:
        print(f'coef: {args.coef}   len: {len(train_loader.dataset)}')
        criterion = lambda output, target: F.cross_entropy(output[0], target) + \
                    args.coef / len(train_loader.dataset) * output[1]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for name, param in model.named_parameters():
            writer.add_histogram(name, param, epoch, bins="auto")
        ...
        writer.add_scalars('precisoin-epoch', {
            'Prec@1': prec1,
            'Prec@5': prec5
            }, epoch)
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        writer.add_scalars('train-{}-log'.format(epoch), {'loss': losses.val, 
                                                    'Prec@1': top1.val, 
                                                    'Prec@5': top5.val}, i)
        ...
        if i % args.print_freq == 0:
            print('Test: [{0}/{1}]\t'
                  ...
                  'Prec@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                   ...
                   top1=top1, top5=top5))
            writer.add_scalars('validate-log', {
                'Loss': losses.val,
                'Prec@1': top1.val,
                'Prec@5': top5.val
                }, i)
...
def save_checkpoint(state, is_best, filename='default'):
    torch.save(state, '{}_checkpoint.pth'.format(filename))
    if is_best:
        shutil.copyfile('{}_checkpoint.pth'.format(filename), '{}_model_best.pth'.format(filename))
